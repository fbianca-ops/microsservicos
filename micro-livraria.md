...
<p align="center">
    <img width="70%" src="./imagens/interface-web.png" />
</p>
...
<p align="center">
    <img width="70%" src="./imagens/arquitetura.png" />
</p>
...
<p align="center">
    <img width="70%" src="./imagens/shipping-proto.png" />
</p>
...
2. Abra a pasta descompactada no VS Code.

3. Abra o terminal do VS Code e instale o Node.js:
...
4. Feche e abra o VS Code novamente. No terminal, verifique se a instalação foi concluída:
...
5. No terminal, dentro da pasta do projeto, instale as dependências:

```powershell
npm install
```

6. Inicie os microsserviços:
...
7.  Para fins de teste, efetue uma requisição para o microsserviço responsável pela API do backend.

- Você pode fazer uma requisição acessando, no seu navegador, a seguinte URL: `http://localhost:3000/products`.

8. Teste agora o sistema como um todo, abrindo o front-end em um navegador: http://localhost:5000. Faça então um teste das principais funcionalidades da livraria.
...
Para ficar claro: até aqui, apenas implementamos a nova operação no backend.


#### Passo 5 — Incorporando a pesquisa ao front-end

Até aqui, a nova operação foi implementada no backend. Agora vamos disponibilizá-la também na interface Web da livraria.

O objetivo é permitir que o usuário informe o ID de um livro e clique em **Pesquisar Livro**. O front-end fará uma requisição REST para o endpoint criado no passo anterior:
...
##### 5.1 Alterando o arquivo `services/frontend/index.html`

No arquivo `services/frontend/index.html`, substitua o trecho:
...
<section class="container">
    <div class="field has-addons">
        ...
        <div class="control">
            <button id="button-search" class="button is-info">Pesquisar Livro</button>
        </div>
        <div class="control">
            <button id="button-list-all" class="button">Listar Todos</button>
        </div>
    </div>
...
O campo `product-id` receberá o identificador do livro. O botão `button-search` iniciará a pesquisa e o botão `button-list-all` permitirá voltar à listagem de todos os livros.

##### 5.2 Alterando o arquivo `services/frontend/index.js`

Para evitar repetir o código responsável por desenhar os livros e adicionar os eventos dos botões, vamos separar essas operações em funções.

Adicione as funções a seguir depois de `calculateShipping`:
...
A função `showBooks` limpa a área de livros da página, adiciona os livros recebidos e, em seguida, associa novamente os eventos dos botões de frete e compra.

Agora adicione uma função para carregar todos os produtos:
...
Em seguida, adicione a função responsável pela nova pesquisa por ID:
...
Observe principalmente esta linha:

```js
fetch('http://localhost:3000/product/' + id)
```

Ela realiza uma requisição HTTP para a rota implementada no `Controller`. Por exemplo, se o usuário digitar `2`, a URL será:

```text
http://localhost:3000/product/2
```

Por fim, substitua o bloco `document.addEventListener('DOMContentLoaded', ...)` existente pelo seguinte:
...
##### 5.3 Testando a alteração

Reinicie os microsserviços, se necessário:

```powershell
npm run start
```

Abra novamente o front-end:

```text
http://localhost:5000
```

Digite, por exemplo, o ID `1` e clique em **Pesquisar Livro**.

O navegador fará a seguinte chamada REST:

```text
GET http://localhost:3000/product/1
```

O `Controller` receberá essa requisição e chamará `SearchProductByID` no microsserviço `Inventory` usando gRPC. O produto retornado será então exibido pelo front-end.

Clique em **Listar Todos** para carregar novamente todos os produtos.
